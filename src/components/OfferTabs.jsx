    ...
    <div>
      <div className={`card${editMode ? ' editing' : ''}`}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
          <div style={{ width: 48, height: 48, borderRadius: '50%', background: partner.color + '22', color: partner.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 500 }}>
            {editMode ? <EditField val={partner.initials} onSave={v => up(p => ({ initials: v }))} style={{ width: 32, textAlign: 'center' }} /> : partner.initials}
          </div>
          <div>
            <div style={{ fontSize: 16, fontWeight: 500 }}>
              {editMode ? <EditField val={partner.name} onSave={v => up(p => ({ name: v }))} style={{ fontSize: 16 }} /> : partner.name}
            </div>
            <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 3, display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div className="grid3" style={{ marginBottom: 10 }}>
          ...
          <div className="metric">
            ...
            <div className="metric-value">{partner.certifications.length || '—'}</div>
          </div>
        </div>
...
export function TabSWOT({ offer, editMode, updateOffer }) {
  ...
  function SwotCell({ title, items, cls, key, sym }) {
    return (
      <div className={`swot-cell ${cls}`}>
        <div className="swot-title">{sym} {title}</div>
        {editMode
          ? <EditList items={items} onSave={v => uo(x => ({ swot: { ...x.swot, [key]: v } }))} placeholder="Ajouter un point..." />
  ...
  return (
    <div className={`card${editMode ? ' editing' : ''}`}>
      <div className="swot-grid">
        <SwotCell title="Forces" items={s} cls="swot-s" key="s" sym="✚" />
        <SwotCell title="Faiblesses" items={w} cls="swot-w" key="w" sym="−" />
        <SwotCell title="Opportunités" items={o} cls="swot-o" key="o" sym="◆" />
        <SwotCell title="Menaces" items={t} cls="swot-t" key="t" sym="▲" />
      </div>
    </div>
  )
}
...
export function TabBattlecard({ offer, editMode, updateOffer }) {
  const uo = fn => updateOffer(fn)
  const cols = ['Nos points forts', 'Leurs points forts', 'Arguments gagnants', 'Pièges à éviter']
