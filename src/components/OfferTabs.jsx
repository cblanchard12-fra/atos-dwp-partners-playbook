  ...
  return (
    <div>
      <div className={`card${editMode ? ' editing' : ''}`}>
        ...
        {/* Pitch partenaire */}
        {(partner.pitch_partner || editMode) && (
          <div style={{ background: 'var(--blue)', borderRadius: 'var(--radius)', padding: '10px 14px', marginBottom: 12 }}>
            {editMode
              ? <EditField val={partner.pitch_partner || ''} onSave={v => up(p => ({ pitch_partner: v }))} multiline rows={2} className="ef-area-white" placeholder="Décrivez l'entreprise et son positionnement en 1-2 phrases..." />
              : <p style={{ fontSize: 12, color: 'white', lineHeight: 1.7, margin: 0 }}>{partner.pitch_partner}</p>}
          </div>
        )}
  ...
  return (
    <div>
      <div className="pitch-box">
        <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8, opacity: 1 }}>
          {editMode
            ? <EditField val={offer.title} onSave={v => uo(o => ({ title: v }))} className="ef-white" style={{ fontSize: 15, fontWeight: 600 }} />
            : offer.title}
        </h3>
        {editMode
