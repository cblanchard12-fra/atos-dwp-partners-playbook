...
export function Sidebar({ data, selPartner, selOffer, onSelectPartner, onSelectOffer, editMode, onToggleEdit, onAddPartner }) {
            ...
            <div key={p.id}>
              ...
              >
                <div
                  ...
                  style={{
                    background: selPartner === p.id ? 'rgba(255,255,255,0.2)' : p.partner.color + '22',
                    color: selPartner === p.id ? 'white' : p.partner.color,
                  }}
                >{p.partner.initials}</div>
                <span className="partner-name">{p.partner.name}</span>
                <span className={`partner-count ${selPartner === p.id ? 'white' : 'gray'}`}>
                  {p.offers.length}
                </span>
              </div>

              {selPartner === p.id && p.offers.map(o => (
                <div
                  key={o.id}
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-row">
              <div className="form-group" style={{ flex: 2 }}>
                ...
                <input className="form-input" value={np.name} onChange={e => setNp(p => ({ ...p, name: e.target.value }))} />
              </div>
              <div className="form-group" style={{ flex: 1 }}>
                <label className="form-label">Initiales</label>
                <input className="form-input" maxLength={2} value={np.initials} onChange={e => setNp(p => ({ ...p, initials: e.target.value.toUpperCase() }))} />
              </div>
            </div>
            <div className="form-row">
              <div className="form-group" style={{ flex: 1 }}>
                <label className="form-label">Couleur</label>
                <input className="form-input" type="color" value={np.color} style={{ height: 34, padding: 2 }} onChange={e => setNp(p => ({ ...p, color: e.target.value }))} />
              </div>
              <div className="form-group" style={{ flex: 2 }}>
                <label className="form-label">Tier</label>
                <select className="form-input" value={np.tier} onChange={e => setNp(p => ({ ...p, tier: e.target.value }))}>
            ...
            <div className="form-row">
              <div className="form-group" style={{ flex: 1 }}>
                ...
                <input className="form-input" value={np.hq} onChange={e => setNp(p => ({ ...p, hq: e.target.value }))} />
              </div>
              <div className="form-group" style={{ flex: 1 }}>
                <label className="form-label">CA</label>
                <input className="form-input" value={np.revenue} onChange={e => setNp(p => ({ ...p, revenue: e.target.value }))} />
              </div>
            </div>
            <div className="form-group">
              <label className="form-label">Titre offre principale</label>
              <input className="form-input" value={np.offerTitle} onChange={e => setNp(p => ({ ...p, offerTitle: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Mots-clés (séparés par virgule)</label>
              <input className="form-input" placeholder="cloud, sécurité..." value={np.keywords} onChange={e => setNp(p => ({ ...p, keywords: e.target.value }))} />
            </div>
